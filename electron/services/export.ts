...
/**
 * The §B.1 IDENTITY MARKER for a workspace's exported primer (the FIRST line of every export
 * file). The marker folds the workspace stem (a real id, or {@link UNTAGGED_STEM}) + schema v1
 * so a regen can confirm "our file, safe to overwrite" AND so E2's adoption scan can find a
 * user `@import` of THIS workspace's primer (an exact-string grep over the host CLAUDE-family
 * files). Claude strips the HTML comment on injection (invisible to Claude, readable by our
 * `readFileSync` scan). SINGLE source — both the exporter and the E2 adoption detector reuse it.
 */
export function workspaceMemoryMarker(stem: string): string {
  return `<!-- mission-control:workspace-memory v1 workspace=${stem} -->`
}
...
export interface ExportEntry {
  ...
  enabled: boolean
  /**
   * CAPP-100 / E2 — the explicit, reversible "I've wired this export into a CLAUDE-family file
   * MYSELF" hint. ONLY a FALLBACK for a Mode-C custom path the user `@import`ed into a host file
   * OUTSIDE our scanned set (so the fresh marker scan can't see it). Never the primary signal;
   * the scan wins. Undefined/false → rely on the scan alone.
   */
  selfWired?: boolean
}
...
export class ExportService {
  ...
  buildFileContent(workspaceId: string | null): string {
    const stem = this.isUntagged(workspaceId) ? UNTAGGED_STEM : (workspaceId as string)
    const marker = workspaceMemoryMarker(stem)
    const provenance =
  ...
  // ── CAPP-100 / E2 — the Mode-C "I wired it myself" hint (adoption fallback) ────────

  /** Read the explicit self-wired hint for a workspace (false when unregistered/unset). The
   *  E2 adoption detector consults this ONLY as a fallback for an unreachable Mode-C path. */
  isSelfWired(workspaceId: string | null): boolean {
    return this.registry.entries[this.keyFor(workspaceId)]?.selfWired === true
  }

  /** Flip the explicit, reversible self-wired hint (the Mode-C "I've wired this myself" toggle).
   *  No-op when the workspace has no export entry. */
  setSelfWired(workspaceId: string | null, selfWired: boolean): ExportState {
    const entry = this.registry.entries[this.keyFor(workspaceId)]
    if (entry) {
      if (selfWired) entry.selfWired = true
      else delete entry.selfWired
      this.persist()
    }
    return this.getExportState(workspaceId)
  }
