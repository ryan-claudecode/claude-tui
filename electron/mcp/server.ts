...
- Workspaces — the durable registry of user-named directory groupings (the spatial frame sessions/missions scope to): list_workspaces / get_active_workspace (read), rescan_workspaces (re-scan the configured paths for new workspace.json manifests and seed them — idempotent, never duplicates or reverts user edits), create_workspace / rename_workspace / add_workspace_dir / remove_workspace_dir / delete_workspace (CRUD by registry id), set_active_workspace (SELECTION-ONLY — mark the active workspace; null clears to the 'All' bucket; does NOT spawn), launch_workspace (the explicit BOOT verb — open editors + spawn one session per repo/dir).
