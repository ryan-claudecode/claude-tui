 ...
 *  workSessions/identity are unused by the registry CRUD tools, so a bare cast
 *  suffices for those. WS-F — `getScanPaths` is injected so the rescan tool can be
 *  driven against a temp scan dir (defaults to none, which keeps the CRUD tests off
 *  the real config). */
