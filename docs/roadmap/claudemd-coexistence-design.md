...
| **I1** | **Inspector v1** — `contextInspector.ts` (tiers 0–7,10; git-root MEMORY.md; `claudeMdExcludes`; comment-strip + fence-skip; "none" for absent; truncation-parity + adoption-aware #10), IPC+preload, panel+route, switcher button, honesty copy. Read-only. | Slice 1, E2 (for #10 adoption awareness — ship #10 in two passes if E2 lags) |
