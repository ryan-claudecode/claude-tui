...
| `electron/services/actionButtons.ts` | **ActionButtonService** (CAPP-104 / AB-1) — agent-generated Agent-Rail action buttons (the rail's FIRST two-way surface): one file per owner at `~/.claude-tui/action-buttons/session-<id>.json` / `workspace-<id>.json` (untagged workspace via the shared `UNTAGGED_STEM` sentinel from workspaceMemory), CRUD with a per-owner CAP of 8 (past it → clear error string), `onChanged` seam → `actionbuttons:updated` push (MAIN window only). A button's action is ALWAYS a PROMPT dispatched to a session (never raw shell) — zero new capability. `deleteForSession` (hooked off `worksession:removed` in ipc.ts) reaps a killed session's buttons; workspace buttons outlive sessions. Pure `dispatchActionButton`/`pickDispatchTerminal` (deps-injected, unit-tested) resolve the click target: reuse the session's most-recent LIVE structured terminal, else spawn a fresh one (`addTerminalToSession`), then deliver via the `sendAgentMessage` stdin sink. Wired Service→IPC (`actionbuttons:*`, `ipc/action-button-handlers.ts`)→preload (main)→MCP (`add_action_button`/`list_action_buttons`/`remove_action_button`, identity-bound to the caller's OWNING session / its workspace, never `getActiveId`). Rail UI = the statically-visible BUTTONS group in `AgentRail.tsx` (pure derivation + two-step-confirm state machine in `src/lib/actionButtonRow.ts`, `useActionButtons` hook mirroring `useSchedules`); the untagged sentinel parity is pinned by `actionButtonUntaggedSync.test.ts`. Design: `docs/roadmap/rail-action-buttons-design.md` (the AB-1 phase) |
| `electron/services/stt.ts` | **SttService** (CAPP-120) — push-to-talk dictation: Parakeet-TDT-0.6b-v2 int8 via `sherpa-onnx-node` in an Electron **utility process** (`electron/stt/sttWorker.ts`, warm recognizer, ~270ms per utterance CPU-only), deps-injected (fork/download/extract/fs are fakes in tests). Model (~680MB, CC-BY-4.0) downloads on first enable to `~/.claude-tui/stt/` — sha256+byte-pinned, cancel/retry/force-re-download, abortable extract, temp-dir cleanup. RPC watchdogs (init 60s / transcribe 30s → teardown+respawn), 3-strike worker-fail ledger surfaced in `status()`. Composer 🎤 (`useDictation` + pure `src/lib/micInteraction.ts`): hold=push-to-talk, click=toggle, Ctrl+M, Esc discards (owns Esc over the agent-interrupt via `dictationEsc.ts`), 5-min cap, insert-at-cursor, never auto-submits. NO MCP tool. Whisper.cpp sidecar = documented fallback only. **Hotwords (CAPP-121):** dictation biases toward YOUR workspace vocabulary — pure `electron/stt/hotwords.ts` (`deriveHotwords`: workspace file names + session findings + model aliases/CLI flags + config `stt.hotwords` extras, cap 300, skip-dirs node_modules/dist/out/build/coverage) written to `hotwords.txt` and fed to Parakeet as real contextual biasing (char-level token encoding, U+2581 boundary rule, score 1.5 — non-destructive); the write is RETRY-SAFE and re-derives when the model download completes (a sticky failed first write would have made biasing silently inert) |
